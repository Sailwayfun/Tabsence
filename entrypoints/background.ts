...
import { urlsStore } from "../src/store/tabUrlMap";
import { firebaseService } from "../src/utils/firebaseService";
import { trackTabTime } from "../src/utils/trackTime";
import { updateUrlDuration } from "../src/utils/trackTime";
import getUserId from "../src/shared/userUtils";
    ...
    chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
      ...
      if (changeInfo.status === "complete" && tab.title !== "Tabsence") {
        ...
        const userId = await chrome.storage.local
          .get("userId")
          .then((res) => res.userId);
        const tabData = await firebaseService.saveNewTabToFirestore(
          tab,
          userId,
        );
        try {
    ...
    chrome.runtime.onMessage.addListener(
      async (message: RuntimeMessage, _, sendResponse) => {
        if (message.action === "updateSpaceTitle") {
          if (!message.userId || !message.spaceId || !message.newSpaceTitle)
            return;

          const { userId, spaceId, newSpaceTitle } = message;
          await firebaseService.updateSpaceTitle(
            userId,
            spaceId,
            newSpaceTitle,
          );
